export const sendManualAlert = async (student: any) => {
  ...
  try {
    // Replace your URL line with this one:
const url = `https://www.fast2sms.com/dev/bulkV2?authorization=${process.env.SMS_API_KEY}&route=otp&variables_values=${encodeURIComponent(message)}&numbers=${student.mobile.replace(/\D/g, '')}`;
